...
void reportGitStatus(String context, String description, String status) {
    ...
    if (env.CHANGE_ID) {
        try {
            githubNotify account: 'Babylonpartners', context: "$context", credentialsId: 'babylonhealth-jenkins-username-password', description: "${description}", gitApiUrl: '', repo: 'babylon-android', sha: "${env.GIT_COMMIT}", status: "${status}", targetUrl: ''
        } catch (error) {
